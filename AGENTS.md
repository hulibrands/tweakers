# AGENTS.md — ShadGPT tweak authoring guide
...
```js
const { defineTweak } = require("@shadgpt/sdk");
...
| `mainStartupTimeoutMs` | `number`              | no       | Main/both-scope async startup cleanup wait override, from `500` to `60000` ms. Defaults to the ShadGPT global setting. |
| `minRuntime`  | `string` (semver range)         | no       | ShadGPT runtime range required. |
...
```json
{
  ...
  "author": {
    "name": "ShadGPT",
    "url": "https://github.com/hulibrands/ShadGPT"
  },
  "homepage": "https://github.com/hulibrands/ShadGPT/tree/main/tweaks/examples/hello-world",
  "tags": ["example", "demo"],
  "scope": "renderer"
}
```
...
See `@shadgpt/sdk` for full types. The most-used pieces:
...
ShadGPT watches your tweak folder and reloads on save. Don't add your own
file watcher — `start()` will simply be re-invoked. Use `stop()` to undo any
DOM mutations / IPC handlers / event listeners.

## Performance rules

Renderer tweaks run inside the same Codex UI process as the app. Treat every
DOM scan and observer callback as user-visible work.

- Scope selectors to the smallest stable root you can find, such as a sidebar,
  menu root, settings page, or message node. Do not start with document-wide
  `div`, `span`, or `p` sweeps.
- Never observe `characterData: true` for streaming assistant text unless the
  feature explicitly needs per-character updates. Prefer `childList` and a
  `requestAnimationFrame` coalescer.
- Cache parsed message or row payloads with `WeakMap` when a structural scan may
  revisit the same node. Invalidate from a small text/signature value.
- For menu integrations, first find bounded menu roots, then query menu items
  inside those roots. Avoid adding bare `div` to menu item selectors.
- Long-lived intervals must be a fallback, not the primary update path. Use
  route changes, mutation observers, storage events, or explicit refresh where
  possible, and back off when the document is hidden.
- Every observer, timer, event listener, and injected node needs cleanup in
  `stop()`.

## Inspecting the live DOM (Chrome DevTools Protocol)

When you're authoring a tweak, you almost always need to know Codex's
real markup — class names, structure, computed styles. The ShadGPT
...
- SDK types: `@shadgpt/sdk` → `TweakManifest`, `TweakApi`,
  `SettingsSection`, etc.
